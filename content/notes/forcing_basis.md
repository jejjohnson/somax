...
> **Status:** Proposed. This note was fact-checked against the somax tree (`core/forcing.py`, `core/terms.py`, `core/model.py`, `domain/domain.py`, `da/vardax_bridge.py`) and the pinned sibling repos. Several integration points the design needs **do not exist yet** and are called out explicitly below rather than assumed. Intended location: `content/notes/forcing_basis.md`. Target module: `somax/_src/core/basis.py`, presets in `somax/_src/core/forcing_bank.py`. The prior layer is a separate concern (companion note `content/notes/forcing_basis_flow_prior.md`, not yet written).
