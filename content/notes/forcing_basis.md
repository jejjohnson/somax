...
4. ✅ Bank presets in `forcing_bank.py`, each composing a geonnax primitive, a temporal feature, and a prior: `ssh_geostrophic` (Gabor frame + wavenumber-law prior), `sss_coastal` (placeable Wendland RBFs + prescribed prior), and `sst_frontal` (box-Laplacian HSGP + Matérn spectral prior), each with an optional Gaussian-window temporal gate via `tile_in_time`. Spectral / data-driven / wavelet builders also landed: `spatial_from_fourier` (Matérn HSGP), `spatial_from_graph_laplacian`, `spatial_from_eof` (DINEOF), `spatial_from_wavelet`, plus the `matern_spectral_density` helper. The two remaining bases are now wired too: `spatial_from_spherical_rbf` (geodesic on-sphere RBF — maps the domain's `(lon, lat)` `coords` to the unit sphere) and the **vector** path — a `VectorSpatialBasis` / `VectorBasisForcing` / `add_vector_to` seam in `basis.py` lets `spatial_from_divfree` + the `geostrophic_currents` preset drive incompressible `(u, v)` current error. All consume the geonnax bases made public in geonnax#25. → verified in `tests/core/test_basis.py` (incl. an analytic divergence-free check and `(u, v)` placement). *Still open:* a time-distributed (windowed) variant of the vector path, and a `global_largescale` spherical-harmonic/Slepian preset.
