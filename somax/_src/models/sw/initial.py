...
def init_h0_jet(domain: Domain, params: SWMParams, u0: Optional[Array]=None):
    ...
    if u0 is not None:
        h_geostrophy = jnp.cumsum(
            - dy
            * x_avg_2D(u0)
            * params.coriolis_param(Y) / params.gravity,
            axis=1
        )
...
def init_h0_zonal_jet(domain):
    ...
    return - jnp.tanh(20.0*((Y-jnp.mean(y))/jnp.max(y)))*400.
...
def init_H_sea_mount(domain: Domain):
    ...
    constant = 9250
    h0 = 100. * jnp.exp(-((X - jnp.mean(x)) ** 2. + (Y - 0.5 * jnp.mean(y)) ** 2.) / (2. * std_mountain ** 2.));
