...
def difference(
    ...
    # kernel size
    kernel_size = tuple(
        calculate_kernel_size(len(coeffs), num_dims, axis=num_dims + axis)
    )
    # initialize strides
    strides = (1,) * num_dims
    # initialize weight
...
def x_diff_1D(
    u: Float[Array, "... Dx"],
    step_size: float | Float[Array, "Dx-1"] = 1.0,
    derivative: int = 1,
    method: str = "backward",
) -> Float[Array, "... Dx-1"]:
    """
...
def y_diff_2D(
    u: Float[Array, "... Dx Dy"],
    step_size: float | Float[Array, "Dy-1"] = 1.0,
    derivative: int = 1,
    method: str = "backward",
) -> Float[Array, "... Dx Dy-1"]:
    """
...
def curl_2D(
    ...
    """
    # ∂xv
    dv_dx = x_diff_2D(u=v, step_size=step_size_x, derivative=1, method=method)
    # ∂yu
    du_dy = y_diff_2D(u=u, step_size=step_size_y, derivative=1, method=method)
